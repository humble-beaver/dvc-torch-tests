...
parser.add_argument('--generate_uts', action='store_true', 
                    help='generate reconstructed frames')
...
def testuvg(test_dataset, global_step, testfull=False):
    with torch.no_grad():
...
def generate_uts(test_dataset, out_folder):
    with torch.no_grad():
        test_loader = DataLoader(dataset=test_dataset, shuffle=False, num_workers=0, batch_size=1, pin_memory=True)
        net.eval()
        cnt = 1
        ...
        for batch_idx, input in enumerate(test_loader):
            print("generating : %d/%d"% (batch_idx, len(test_loader)))
            input_images = input[0]
            ref_image = input[1]
            seqlen = input_images.size()[1]
            for i in range(seqlen):
                input_image = input_images[:, i, :, :, :]
                inputframe, refframe = Var(input_image), Var(ref_image)
                clipped_recon_image, _, _, _, _, _, _, _ = net(inputframe, refframe)
                im_name = f"{out_folder}/img{str(cnt).zfill(4)}.png"
                print(f"saving {im_name}")
                cnt += 1
                if os.path.isfile(im_name):
                    print(f"{im_name} already exists!")
                    exit(0)
                save_image(clipped_recon_image, im_name)
                print(f"cnt: {cnt}")
...
if __name__ == "__main__":
    ...
    if args.testuvg:
        ...
        print('testing UVG')
        testuvg(test_dataset, 0, testfull=True)
        exit(0)
    
    if args.generate_uts:
        test_dataset = UVGDataSet(refdir=ref_i_dir, testfull=True)
        print('generating UTS frames')
        generate_uts(test_dataset, args.out_folder)
        exit(0)
