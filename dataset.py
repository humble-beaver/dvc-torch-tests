...
class UVGDataSet(data.Dataset):
        ...
        for folder in folders:
            ...
            for i in range(framerange):
                refpath = os.path.join(
                    root, seq, refdir, 'im'+str(i * 12 + 1).zfill(4)+'.png')
                inputpath = []
                for j in range(12):
                    inputpath.append(os.path.join(
                        root, seq, 'im' + str(i * 12 + j + 1).zfill(3)+'.png'))
    ...
    def getbpp(self, ref_i_folder):
        ...
        if ref_i_folder == 'H265L20':
            ...
            # you need to fill bpps after generating crf=20
            Ibpp = [0.19377278645833335]
        elif ref_i_folder == 'H265L23':
            print('use H265L23')
            # you need to fill bpps after generating crf=23
            Ibpp = [0.38544596354166666, 0.10952189127604169,
                    0.19802001953125, 0.23154418945312502,
                    0.13851847330729167, 0.11890665690104167,
                    0.37852620442708335]
        elif ref_i_folder == 'H265L26':
            print('use H265L26')
            # you need to fill bpps after generating crf=26
            Ibpp = [0.14656412760416665, 0.05508504231770833,
                    0.13444051106770832, 0.15891520182291669,
                    0.08679443359375001, 0.06904581705729167,
                    0.25536661783854164]
        elif ref_i_folder == 'H265L29':
            print('use H265L29')
            # you need to fill bpps after generating crf=29
            Ibpp = [0.055919189453125, 0.03423339843750001,
                    0.093309326171875, 0.10747965494791667,
                    0.05423828125, 0.044903564453125,
                    0.17180053710937498]
        else:
    ...
    def __getitem__(self, index):
        ref_image = imageio.imread(self.ref[index]).transpose(
        ...
        for filename in self.input[index]:
            input_image = (imageio.imread(filename).transpose(
                2, 0, 1)[:, :h, :w]).astype(np.float32) / 255.0
            if refpsnr is None:
                refpsnr = CalcuPSNR(input_image, ref_image)
                refmsssim = ms_ssim(torch.from_numpy(input_image[np.newaxis, :]), torch.from_numpy(
                    ref_image[np.newaxis, :]), data_range=1.0).numpy()
            else:
...
class DataSet(data.Dataset):
    def __init__(self, rdir="data/vimeo_septuplet/sequences/", folderlist="data/vimeo_septuplet/test.txt", im_height=256, im_width=256):
        self.image_input_list, self.image_ref_list = self.get_vimeo(rootdir=rdir,
                                                                    filefolderlist=folderlist)
        self.im_height = im_height
        self.im_width = im_width

        self.featurenoise = torch.zeros(
            [out_channel_M, self.im_height // 16, self.im_width // 16])
        self.znoise = torch.zeros(
            [out_channel_N, self.im_height // 64, self.im_width // 64])
        self.mvnois = torch.zeros(
            [out_channel_mv, self.im_height // 16, self.im_width // 16])
    ...
    def __getitem__(self, index):
        ...
        quant_noise_feature, quant_noise_z, quant_noise_mv = torch.nn.init.uniform_(torch.zeros_like(self.featurenoise), -0.5, 0.5), torch.nn.init.uniform_(
